...
export function TaskCard({ task }: { task: Task }) {
  ...
  async function cycleStatus() {
    ...
    const idx = order.indexOf(optimistic);
  ...
  return (
    <div
      className={`group relative rounded-btn border border-white/5 bg-black/20 p-3 transition-all hover:bg-black/30 hover:border-white/10 ${
      ...
      <div className="flex items-start gap-2.5">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-2 flex flex-wrap items-center gap-1.5">
            ...
            {task.assignee?.display_name && <Chip tone="purple">{task.assignee.display_name}</Chip>}
            {task.due_at && (
              <span
                className={`text-[11px] ${
        ...
        <button
          ...
          className="opacity-0 transition-opacity group-hover:opacity-100 text-xs text-[var(--muted)] hover:text-white"
        >
          ⟳
        </button>
      </div>
